...
# Use TEST_X_USER_API_KEY which should be valid in the database
TEST_API_KEY = env.get('TEST_X_USER_API_KEY') or env.get('TEST_X_PAPR_API_KEY')
...
class TestGraphQLProxy:
    ...
    def test_graphql_endpoint_with_api_key(self, app):
        ...
        if not TEST_API_KEY:
            pytest.skip("TEST_X_USER_API_KEY not set in .env")
        ...
        with TestClient(app) as client:
            ...
            # Mock only Neo4j GraphQL response (use real authentication)
